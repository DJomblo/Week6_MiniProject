...
// Main Activity hosting the RecyclerView to display recipe list
class MainActivity : AppCompatActivity() {

    // Lazy init RecyclerView reference
    private val recyclerView: RecyclerView by lazy { findViewById(R.id.recycler_view) }

    // Adapter that handles recipe data and image loading with click callbacks
    // We pass layoutInflater, an ImageLoader implementation (GlideImageLoader), and a click callback.
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Setup RecyclerView with adapter and linear layout manager
        recyclerView.adapter = recipeAdapter
        recyclerView.layoutManager = LinearLayoutManager(this)

        // Load sample recipe data into adapter (each recipe has ingredients and steps)
        recipeAdapter.setData(
            listOf(
                ...
                // Indonesian recipe - Beef Rendang (matching ingredients: List<String>, and steps String)
                RecipeModel(
    ...
    // Show AlertDialog to display recipe details on item click
    // Includes a nicely formatted ingredients list + steps
    private fun showRecipeDialog(recipe: RecipeModel) {
        // Build ingredients text from list (safe even if list is empty)
